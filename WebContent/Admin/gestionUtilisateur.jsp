...
<body class="grey lighten-3">
  ...
  <main class="pt-5 mx-lg-5">
        <div class="col-sm-12 table-wrapper">
            ...
            <table class="table table-striped table-hover">
                ...
                <c:set var="idUser" scope="page" value="${ utilisateur.id }"/>
                    <tr>
                        <td>
                          <span class="custom-checkbox">
                          </span>
                        ...
                        <td><c:out value="${ utilisateur.firstName }"/></td>
                        <td><c:out value="${ utilisateur.lastName }"/></td>
                        <td><c:out value="${ utilisateur.password }"/></td>
                        <td><c:out value="${ utilisateur.status }"/></td>
                        <td>
                            <a href="#editEmployeeModal<c:out value="${ idUser }"/>" class="edit" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i></a>
                            <a href="#deleteEmployeeModal<c:out value="${ idUser }"/>" class="delete" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Delete">&#xE872;</i></a>
                        </td>
                    </tr>
                    
                    <!-- Edit Modal HTML -->
  <div id="editEmployeeModal<c:out value="${ idUser }"/>" class="modal fade">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="post" action="../album/update">
          ...
          <div class="form-group">
              ...
              <input type="text" name="idUsers" class="form-control" value="<c:out value="${ utilisateur.id }"/>" hidden>
              <input type="text" name="passwordUser" class="form-control" value="<c:out value="${ utilisateur.password }"/>" >
            </div>
            <div class="form-group">
              <label>Login</label>
              <input type="text" name="loginUser" class="form-control" value="<c:out value="${ utilisateur.login }"/>" required>
            </div>
            <div class="form-group">
              <label>FirstName</label>
              <input type="text" name="firstNameUser" class="form-control" value="<c:out value="${ utilisateur.firstName }"/>" required>
            </div>
            <div class="form-group">
              <label>LastName</label>
              <input type="text" name="lastNameUser" class="form-control" value="<c:out value="${ utilisateur.lastName }"/>" required>
            </div>
            
           
            <div class="form-check">
              <input type="radio" class="form-check-input" name="typeUser" id="ADMIN" value="ADMIN" 
              <c:if test="${utilisateur.getStatus() == \"ADMIN\"}">   
              checked
              </c:if>>
              <label for="prive" class="form-check-label">Admin</label>
            </div>
            <div class="form-check">
              <input type="radio" class="form-check-input" name="typeUser" id="USER" value="USER"
               <c:if test="${utilisateur.getStatus() == \"USER\"}">   
              checked
              </c:if>>
  ...
  <div id="addEmployeeModal" class="modal fade">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="post" action="../album/addUser">
          <div class="modal-header">
  ...
  <!-- Delete Modal HTML -->
  <div id="deleteEmployeeModal<c:out value="${ idUser }"/>" class="modal fade">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="post" action="../album/deleteUser">
          <div class="modal-header">
            <h4 class="modal-title">Delete Employee</h4>
            <input type="text" name="idUserDel" class="form-control" value="<c:out value="${ utilisateur.id }"/>" hidden>
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
          </div>
          <div class="modal-body">
